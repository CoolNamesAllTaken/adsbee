...
UTEST(SPICoprocessor, ReadWriteReadRewriteRereadBigNoAck) {
    ...
    ASSERT_TRUE(pico.Read(ObjectDictionary::Address::kAddrSettingsStruct, settings_in_original));
    SettingsManager::Settings settings_out;
    for (uint16_t i = 0; i < sizeof(settings_out); i++) {
        ((uint8_t *)&settings_out)[i] = i % UINT8_MAX;
    }
    ASSERT_TRUE(pico.Write(ObjectDictionary::Address::kAddrSettingsStruct, settings_out));
    SettingsManager::Settings settings_in_modified;
    ASSERT_TRUE(pico.Read(ObjectDictionary::Address::kAddrSettingsStruct, settings_in_modified));
    for (uint16_t i = 0; i < sizeof(settings_out); i++) {
        EXPECT_EQ(i % UINT8_MAX, ((uint8_t *)&settings_in_modified)[i]);
    }
    ASSERT_TRUE(pico.Write(ObjectDictionary::Address::kAddrSettingsStruct, settings_in_original));
}
...
UTEST(SPICoprocessor, ReadWriteReadRewriteRereadBigWithAck) {
    ...
    ASSERT_TRUE(pico.Write(ObjectDictionary::Address::kAddrSettingsStruct, settings_out, true));
    SettingsManager::Settings settings_in_modified;
    ASSERT_TRUE(pico.Read(ObjectDictionary::Address::kAddrSettingsStruct, settings_in_modified));
    for (uint16_t i = 0; i < sizeof(settings_out); i++) {
        EXPECT_EQ(i % UINT8_MAX, ((uint8_t *)&settings_in_modified)[i]);
    }
    ASSERT_TRUE(pico.Write(ObjectDictionary::Address::kAddrSettingsStruct, settings_in_original, true));
}
