...
UTEST(Si4362, ReadPartInfo) {
    EXPECT_TRUE(adsbee.subg_radio.SendCommand(Si4362::Command::kCmdNop));
    uint8_t part_info_buf[8] = {0};
    EXPECT_TRUE(adsbee.subg_radio.ReadCommand(Si4362::Command::kCmdPartInfo, part_info_buf, sizeof(part_info_buf)));
...
UTEST(Si4362, SetGetProperty) {
    ...
    EXPECT_TRUE(adsbee.subg_radio.SetProperty(Si4362::kGroupPreamble, 4, 0x05, preamble_config));
    uint8_t read_buf[4] = {0};
    EXPECT_TRUE(adsbee.subg_radio.GetProperty(Si4362::kGroupPreamble, 4, 0x05, read_buf));
...
UTEST(Si4362, GetDeviceState) {
    ...
    EXPECT_TRUE(adsbee.subg_radio.GetDeviceState(state, channel));
    printf("Device state: %s\r\n", adsbee.subg_radio.DeviceStateToString(state));
    printf("Channel: %d\r\n", channel);
}
...
UTEST(Si4362, GetSyncWord) {
    uint8_t data[6] = {0};
    EXPECT_TRUE(adsbee.subg_radio.GetProperty(Si4362::kGroupSync, sizeof(data), 0x00, data));
...
UTEST(Si4362, GetModemDataRate) {
    ...
    EXPECT_TRUE(adsbee.subg_radio.GetModemConfig(modem_config));
    modem_config.print();
}