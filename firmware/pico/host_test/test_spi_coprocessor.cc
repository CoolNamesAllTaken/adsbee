...
TEST(SPICoprocessor, CreateSCpacket) {
    SPICoprocessor::SCPacket packet = {.crc = 0x1234, .length = 0};
    EXPECT_FALSE(packet.IsValid(sizeof(SPICoprocessor::SCPacket)));

    packet.PopulateCRCAndLength(0);
    EXPECT_TRUE(packet.IsValid(sizeof(SPICoprocessor::SCPacket)));
}
...
TEST(SPICoprocessor, CreateSettingsPacket) {
    ...
    SPICoprocessor::SettingsPacket packet = SPICoprocessor::SettingsPacket(settings);
    EXPECT_TRUE(packet.IsValid(sizeof(SPICoprocessor::SettingsPacket)));
    ...
    EXPECT_FALSE(packet.IsValid(sizeof(SPICoprocessor::SettingsPacket)));
}
...
TEST(SPICoprocessor, CreateAircraftListPacket) {
    ...
    SPICoprocessor::AircraftListPacket packet = SPICoprocessor::AircraftListPacket(num_aircraft, aircraft_list);
    EXPECT_TRUE(packet.IsValid(sizeof(SPICoprocessor::AircraftListPacket)));
    ...
    EXPECT_FALSE(packet.IsValid(sizeof(SPICoprocessor::AircraftListPacket)));
}