...
inline uint16_t WriteCSBeeAircraftMessageStr(char message_buf[], const Aircraft &aircraft) {
    ...
    uint16_t num_chars = sprintf(
        message_buf, "#A:%000000X,%X,%s,%04o,%.5f,%.5f,%d,%d,%.0f,%.0f,%d,%d,%d", aircraft.icao_address, aircraft.flags,
        aircraft.callsign, aircraft.squawk, aircraft.latitude_deg, aircraft.longitude_deg, aircraft.baro_altitude_ft,
        aircraft.gnss_altitude_ft, aircraft.track_deg, aircraft.velocity_kts, aircraft.vertical_rate_fpm,
        aircraft.last_message_signal_strength_dbm, aircraft.last_message_signal_quality_db);
