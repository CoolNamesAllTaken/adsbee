...
class CommsManager {
   public:
     ...
     * @param log_level The log level of the message. Messages are ignored if they are lower priority than the current
     * log level.
    ...
    //     PFBQueue<Decoded1090Packet>(PFBQueue<Decoded1090Packet>::PFBQueueConfig{.buf_len_num_elements =
    //     SettingsManager::Settings::kMaxNumTransponderPackets,
    //                                                                             .buffer =
    //                                                                             transponder_packet_reporting_queue_buffer_});
   ...
   private:
    ...
    // Decoded1090Packet
    // transponder_packet_reporting_queue_buffer_[SettingsManager::Settings::kMaxNumTransponderPackets];
};
...
#define CONSOLE_PRINTF(...) comms_manager.LogMessageToCoprocessor(SettingsManager::LogLevel::kSilent, "", __VA_ARGS__);
