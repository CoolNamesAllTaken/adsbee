...
// This low-level driver interacts with the high-level driver since it needs to do async callback stuff, which
// necessitates doing tasks that are normally done from a blocking implementation of the high level driver.
#include <cassert>
...
void spi_transfer_complete_callback(SPI_Handle handle, SPI_Transaction* transaction) {
    bool parse_message =
        !pico_ll.SPIIsUsingHandshakePin();  // Don't parse garbage sent by the RP2040 when it's reading our reply.
...
Pico::Pico(PicoConfig config_in) : config_(config_in) {
    ...
    // spi_params_.bitRate = 4'000'000; // Not used in slave mode but needs to be reasonable since it's used to set a
    // clock.
...
bool Pico::SPIWriteNonBlocking(uint8_t* tx_buf, uint16_t len_bytes) {
    spi_transaction_.count =
        kSPITransactionMaxLenBytes;  // We want the transaction callback to be called as soon as chip select goes high.
                                     // Set the max length we might see so the transaction doesn't terminate early.
...
bool Pico::SPIProcessTransaction() {
    if (spi_transaction_.status != SPI_STATUS_SUCCESS && spi_transaction_.status != SPI_TRANSFER_CSN_DEASSERT) {
        CONSOLE_ERROR("Pico::SPIPostTransactionCallback", "SPI transaction status is not success: %d.",
                      spi_transaction_.status);
        SPIResetTransaction();
        return false;
    }
    ...
    switch (cmd) {
        ...
        case ObjectDictionary::SCCommand::kCmdWriteToSlaveRequireAck: {
            ...
            if (cmd == ObjectDictionary::SCCommand::kCmdWriteToSlaveRequireAck) {
                ...
                SPIResetTransaction();  // Re-arm DMA; no ACK response will trigger the re-arm path.
            }
            break;
        }
