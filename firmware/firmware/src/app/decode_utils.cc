...
/**
 * @brief Converts a numeric value into an ASCII character for use in decoding callsigns.
...
/**
 * @brief Calculate the number of longituide zones (between 1 and 59) at a given latitude.
...
uint16_t calc_nl_cpr_from_lat(float lat) {
    ...
    return floorf(
        2.0f*(float)M_PI / 
        acosf(1 - 
            (1-cosf((float)M_PI/(2.0f*kCPRNz))) / 
            powf(cosf((float)M_PI/180.0f*lat), 2)
        )
    );
}
