 ...
 * @param[in] first_bit_index Bit index begin reading from (index of MSb of word to read). MSb of first word in buffer is bit 0.
...
/**
 * @brief Insert an n-bit word into a big-endian buffer of 32-bit words. Does NOT guard against falling off the end of
 * the buffer, so be careful!
...
void set_n_bit_word_in_buffer(uint16_t n, uint32_t word, uint32_t first_bit_index, uint32_t buffer[]) {
    if (n > WORD_32_NUM_BITS || n < 1) {
        printf("set_n_bit_word_in_buffer: Tried to set %d word in buffer, but word bitlength must be between 1 and 32.\r\n");
        return;
    }
